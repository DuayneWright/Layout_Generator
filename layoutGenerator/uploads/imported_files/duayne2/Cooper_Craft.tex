    ...
    \cfoot{}
    \rfoot{\textbf{\LARGE Jasmine Flores}\\\vspace{3pt}{\large\color{gray}NIH STTR Phase II}}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[scale=1/5.1000000000000005,  % use this to reduce or enlargen the image on paper
                               x=1cm,
                               y=1cm,
        ...
        %% GRID - X
        \foreach \i in {-36.0,0.0,...,36.0}{
            \draw[grid-line] (\i,-5.0) -- (\i,41.0);
            \tikzmath{int \value; \value = \i;}; 
            \node[gray, below] at (\i,-5.0) {\SI{\value}{\inch}};
        }

        %% GRID - Y
        \foreach \i in {0.0,36.0,...,36.0}{
            \draw[grid-line] (-41.0,\i) -- (41.0,\i);
            \tikzmath{int \value; \value = \i;};
            \node[gray, left] at (-41.0,\i) {\SI{\value}{\inch}};
        }
    ...
    % Cameras
    \camera[color=blue!75!black,rotate=90.0, inner sep=7.5mm](-2.55,23.34);
\camera[color=blue!75!black,rotate=0.0, inner sep=7.5mm](2.45,9.64);


    % Sensors
    \node[sensor, color=teal, scale=1.0, inner sep=7.5mm](sensor) at (-10.3,10.84) {};
\node[sensor, color=teal, scale=1.0, inner sep=7.5mm](sensor) at (-26.8,29.64) {};

    
    % Calibrations
    \node[location, color=violet, scale=1.0, inner sep=7.5mm]( ) at (4.45,11.34) {};
\node[location, color=violet, scale=1.0, inner sep=7.5mm]( ) at (26.95,26.89) {};
	...
	\begin{tabular}{l l}
		\makecell[lt]{
			\textbf{Location:}\\
     		Dolorem est fugiat  Neighborhood \\
 		    Voluptatem Omnis cu Bldg, Still Hopes\\
           	1 Still Hopes Drive\\
