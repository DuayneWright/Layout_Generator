    ...
    \lhead{2003 August 28}
    \chead{}
    \rhead{\footnotesize \thepage \ {\color{gray} of \pageref{LastPage}}}

    \cfoot{}
    \rfoot{\textbf{\LARGE Penelope Andrews}\\\vspace{3pt}{\large\color{gray}NIH STTR Phase II}}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[scale=1/3.4,  % use this to reduce or enlargen the image on paper
                               x=1cm,
                               y=1cm,
        ...
        %% GRID - X
        \foreach \i in {-24.0,0.0,...,24.0}{
            \draw[grid-line] (\i,-29.0) -- (\i,29.0);
            \tikzmath{int \value; \value = \i;}; 
            \node[gray, below] at (\i,-29.0) {\SI{\value}{\inch}};
        }

        %% GRID - Y
        \foreach \i in {-24.0,0.0,...,24.0}{
            \draw[grid-line] (-29.0,\i) -- (29.0,\i);
            \tikzmath{int \value; \value = \i;};
            \node[gray, left] at (-29.0,\i) {\SI{\value}{\inch}};
        }
    ...
    % Room Navigation
    \draw[nav-arrow,color=green!60!black,{Stealth[scale=1.0]}-] (11.88,0.13) -- ++(-12.0,1.4695761589768238e-15);
\draw[nav-arrow,color=green!60!black,{Stealth[scale=1.0]}-] (-0.13,12.88) -- ++(-2.204364238465236e-15,-12.0);
\draw[nav-arrow,color=green!60!black,{Stealth[scale=1.0]}-] (-11.38,0.13) -- ++(12.0,-0.0);
\draw[nav-arrow,color=green!60!black,{Stealth[scale=1.0]}-] (-0.13,-10.38) -- ++(7.347880794884119e-16,12.0);
\draw[nav-arrow,color=green!60!black,{Stealth[scale=1.0]}-] (-6.88,-6.63) -- ++(8.485281374238571,8.48528137423857);
\draw[nav-arrow,color=green!60!black,{Stealth[scale=1.0]}-] (7.38,-7.38) -- ++(-8.48528137423857,8.485281374238571);
\draw[nav-arrow,color=green!60!black,{Stealth[scale=1.0]}-] (8.38,8.63) -- ++(-8.485281374238571,-8.48528137423857);
\draw[nav-arrow,color=green!60!black,{Stealth[scale=1.0]}-] (-6.88,6.88) -- ++(8.485281374238554,-8.485281374238587);
	...
	\begin{tabular}{l l}
		\makecell[lt]{
			\textbf{Location:}\\
     		Impedit consectetur Neighborhood \\
 		    Repudiandae laborum Bldg, Still Hopes\\
           	1 Still Hopes Drive\\
