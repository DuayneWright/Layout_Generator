    ...
    \lhead{2001 February 12}
    \chead{}
    \rhead{\footnotesize \thepage \ {\color{gray} of \pageref{LastPage}}}

    \cfoot{}
    \rfoot{\textbf{\LARGE MacKensie Clemons}\\\vspace{3pt}{\large\color{gray}NIH STTR Phase II}}
    ...
    \begin{figure}[H]
        ...
        %% GRID - X
        \foreach \i in {0.0,24.0,...,120.0}{
            \draw[grid-line] (\i,-5.0) -- (\i,197.0);
            \tikzmath{int \value; \value = \i;}; 
            \node[gray, below] at (\i,-5.0) {\SI{\value}{\inch}};
        }

        %% GRID - Y
        \foreach \i in {0.0,24.0,...,192.0}{
            \draw[grid-line] (-5.0,\i) -- (125.0,\i);
            \tikzmath{int \value; \value = \i;};
            \node[gray, left] at (-5.0,\i) {\SI{\value}{\inch}};
        }
    

    \draw[wall, line width=2pt, color=black] (13.50,0.00) -- (0.00,0.00);
\draw[wall, line width=2pt, color=black] (0.00,0.00) -- (0.00,169.25);
\draw[wall, line width=2pt, color=black] (0.00,169.25) -- (115.50,169.25);
	...
	\begin{tabular}{l l}
		\makecell[lt]{
			\textbf{Location:}\\
     		Ratione rerum amet  Neighborhood \\
 		    Ut non fugit aut fa Bldg, Still Hopes\\
           	1 Still Hopes Drive\\
