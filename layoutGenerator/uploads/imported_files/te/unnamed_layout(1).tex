    ...
    \lhead{2025 February 14}
    \chead{}
    \rhead{\footnotesize \thepage \ {\color{gray} of \pageref{LastPage}}}

    \cfoot{}
    \rfoot{\textbf{\LARGE r}\\\vspace{3pt}{\large\color{gray}NIH STTR Phase II}}
	...
	\draw[line width=0pt, color=black, rotate around={#3:(#4,#5)}] (#4-#2/2, #5-#1/2) rectangle (#4+#2/2, #5+#1/2)  
    }
    ...
    \def\furnitureC[radius=#1](#2, #3){%
	    ...
	    \draw[line width=0pt, color=black] (#2, #3) circle (#1)
    }
    ...
    \node[location, color=violet, scale=2.0]( ) at (1.0,1.0) {};
