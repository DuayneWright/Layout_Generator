    ...
    \cfoot{}
    \rfoot{\textbf{\LARGE Rama Kelly}\\\vspace{3pt}{\large\color{gray}NIH STTR Phase II}}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[scale=1/10,  % use this to reduce or enlargen the image on paper
        ...
        %% GRID - Y
        \foreach \i in {0.0,36.0,...,36.0}{
            \draw[grid-line] (-5.0,\i) -- (41.0,\i);
            \tikzmath{int \value; \value = \i;};
            \node[gray, left] at (-5.0,\i) {\SI{\value}{\inch}};
        }
    

    \camera[color=blue!75!black,rotate=0.0](12.0,12.0);
\node[camera-label, text=blue!75!black, scale=1.0] at (12.0,12.0) {first};
\node[location, color=violet, scale=1.0](second) at (22.0,12.0) {};
\node[location-label, text=violet, scale=1.0] at (second) {second};
\node[sensor, color=teal, scale=1.0](sensor) at (12.0,22.0) {};
\node[sensor-label,text=teal,scale=1.0] at (12.00,22.00) {third \\ (12.00, 22.00)};
	...
	\begin{tabular}{l l}
		\makecell[lt]{
			\textbf{Location:}\\
     		Non nostrum a minus  Neighborhood \\
 		    Necessitatibus accus Bldg, Still Hopes\\
           	1 Still Hopes Drive\\
