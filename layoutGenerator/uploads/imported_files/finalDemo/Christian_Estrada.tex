    ...
    \cfoot{}
    \rfoot{\textbf{\LARGE Michelle Cherry}\\\vspace{3pt}{\large\color{gray}NIH STTR Phase II}}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[scale=1/7.6,  % use this to reduce or enlargen the image on paper
                               x=1cm,
                               y=1cm,
        ...
        %% GRID - X
        \foreach \i in {-72.0,-48.0,...,24.0}{
            \draw[grid-line] (\i,-77.0) -- (\i,53.0);
            \tikzmath{int \value; \value = \i;}; 
            \node[gray, below] at (\i,-77.0) {\SI{\value}{\inch}};
        }

        %% GRID - Y
        \foreach \i in {-72.0,-48.0,...,48.0}{
            \draw[grid-line] (-77.0,\i) -- (29.0,\i);
            \tikzmath{int \value; \value = \i;};
    ...
    % Exterior walls
    \draw[wall, line width=2pt, color=black] (-33.79,10.07) -- (-9.00,10.25);
\draw[wall, line width=2pt, color=black] (-14.04,-26.43) -- (10.75,-26.25);
\draw[wall, line width=2pt, color=black] (-49.25,-20.75) -- (-7.00,-63.00);
    ...
    % Sensors
    \node[sensor, color=teal, scale=1.0, inner sep=7.5mm](sensor) at (-22.54,47.57) {};
\node[sensor-label,text=teal,scale=1.0, fill=none, inner sep=2pt] at (-22.54,47.57) {obj2 \\ (-22.54, 47.57)};

    
    % Calibrations
    \node[location, color=violet, scale=1.0, inner sep=7.5mm](obj1) at (6.71,37.32) {};
\node[location-label, text=violet, scale=1.0, fill=none, inner sep=2pt] at (obj1) {obj1};
	...
	\begin{tabular}{l l}
		\makecell[lt]{
			\textbf{Location:}\\
     		Est in quibusdam co Neighborhood \\
 		    Aut in ad aut vel ve Bldg, Still Hopes\\
           	1 Still Hopes Drive\\
