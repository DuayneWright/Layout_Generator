...
class UpdateFileNameForm(ModelForm):
    ...
    def clean_new_file_name(self):
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', new_file_name):
            raise forms.ValidationError("File name can only contain letters, numbers, underscores, and hyphens.")
...
class UpdateStyleSettingsForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'furniture_width': forms.TextInput(attrs={'class': 'form-control', 'type': 'number', 'min': 0.5, 'max': 16, 'step': '0.5'}),
    ...
    def clean(self):
            ...
            if furniture_width is not None and (furniture_width < 0.5 or furniture_width > 16):
                raise ValidationError('Furniture width must be between 0.5 and 16.')
