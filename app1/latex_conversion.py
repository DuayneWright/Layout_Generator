...
def conversion(file, layout_style, canvas=False):
    """
    ...
    num_type_cols = ['x', 'y', 'width', 'length', 'rotation', 'radius', 'min', 'max', 'step']
    str_type_cols = ['type', 'descriptor', 'furniture_type']
    ...
    # Check wall/window bounds and extend layout if needed
    wall_window_data = excel_data[excel_data['type'].isin(['wall', 'window'])]
    toggle_pair = False

    for index, row in wall_window_data.iterrows():
        x1 = row['x']
        y1 = row['y']
        toggle_pair = not toggle_pair
        
        if toggle_pair and index < len(wall_window_data) - 1:
            x2 = wall_window_data.iloc[index + 1]['x']
            y2 = wall_window_data.iloc[index + 1]['y']
            
            # Extend x bounds
            min_x = min(x1, x2)
            max_x = max(x1, x2)
            if min_x < x_min:
                x_min = x_step * math.floor(min_x/x_step)
            if max_x > x_max:
                x_max = x_step * math.ceil(max_x/x_step)
                
            # Extend y bounds    
            min_y = min(y1, y2)
            max_y = max(y1, y2)
            if min_y < y_min:
                y_min = y_step * math.floor(min_y/y_step)
            if max_y > y_max:
                y_max = y_step * math.ceil(max_y/y_step)


    # # Check wall bounds and extend layout if needed
    # wall_data = excel_data[excel_data['type'] == 'wall']
    # for _, row in wall_data.iterrows():
    ...
    # Check furniture bounds and extend layout if needed for left side 
    ...
    for index, row in excel_data.iterrows():
        ...
        #     continue
            ...
            if type(door_xy) in [str]:
                ...
                if(door_xy == 'left' and x - row['length']*cos_theta >= x_min and y - row['length']*sin_theta >= y_min):
                    ...
                    message = f"Invalid input for door length. Please ensure the door fits within the layout grid."
                    print(message)  
                    return {'success': False, 'message': message}
            ...
            y = row['y']
            scale = row['scale']
            #TODO: add check for style line width. If 0.5 or less, add in a latex_..._furniture_default_template with no scale (to force default)
            if pd.isna(scale) or pd.isnull(scale):
                scale = 1.0
            ...
            if canvas:
                latex_cameras += latex_camera_template.format(rotation, x, y, width=width)
        ...
        elif row_type.lower() == 'room navigation':
            x = row['x']
            y = row['y']
            room_navigation_direction = row['room_navigation_direction'].lower() if not pd.isna(row['room_navigation_direction']) else 'right'
            scale = row['scale']
            rotation = row['rotation']
    ...
    print("**********************************", "denom: ", denom, "\touter_bound: ", outer_bound, "\tgap: ", gap, "\tadjusted_multiplier: ", adjusted_multiplier, "\touter_bound / gap: ", outer_bound / gap)
    if denom == 0:
...
def ensure_header(excel_data):
    ...
    required_cols = ['type', 'descriptor', 'x', 'y', 'width', 'length', 'radius', 'scale', 'rotation',
                     'furniture_type', 'room_navigation_direction', 'min', 'max', 'step']
